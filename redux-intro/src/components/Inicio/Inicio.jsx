      ...
      <ul>
        {proyectos &&
          proyectos.map((proyecto) => (
            <li key={proyecto._id}>
              <Card style={{ marginBottom: '16px',  maxWidth:"90%", margin:"auto" }}>
                <div className="title-row" style={{ backgroundColor: "#E99C5D", padding: "8px"}}>
                  <Typography variant="h6" style={{ color: '#fff', textAlign: 'center' }}>{proyecto.Titulo}</Typography>
                </div>
                <CardContent style={{ padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ marginBottom: '16px', textAlign: 'center' }}>
                    <Typography variant="subtitle1">Estado: {proyecto.Estado}</Typography>
                    <Typography variant="subtitle1">Empresa: {getEmpresaName(proyecto?.IdEmpresa)}</Typography>
                    <Typography variant="body1">Descripción: {proyecto.Descripcion}</Typography>
                  </div>
                </CardContent>
                <CardActions disableSpacing style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '8px' }}>
                    <Button onClick={() => handleEnviarSolicitud(proyecto._id)} className="solicitud-button" style={{ backgroundColor: "#75a5c5", color: '#fff', margin: '8px' }}> {/* Aplicando el color azul y texto blanco */}
                      {proyecto?.Solicitudes[0] ? 'Solicitud Enviada' : 'Enviar Solicitud'}
                    </Button>
                  </div>
                ...
                <Collapse in={expandedIds.includes(proyecto._id)} timeout="auto" unmountOnExit>
